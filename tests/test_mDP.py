...
from HREG import HREG,LapHREG
from UPHREG import UPHREG,ExpUPHREG,LapUPHREG
import numpy as np
...
def build_DP_model(vecfile_path,model_path,epsilon=0.2):
...
def build_DP_from_file(dataset,exp):
    test_name = "epsilon"
    ...
    for epsilon in [1,5,10,15,20]:
        ...
        if(os.path.basename(h_path) not in os.listdir(test_path)):
            build_DP_model(vecfile_path,h_path,epsilon)
...
def test_base(dataset,exp)->None:
    model_dir=os.path.join(f"{dataset}_{class_name}",f"10^{exp}")   
    K = 100
    ef= 200
    avg = 0
...
if __name__ == "__main__":
    # clean_test_res()
    for dataset in ["randvec","DEEP","GloVe","SIFT"]:
        for exp in [3,4]:
            plt.ylim(0,1.1)
            for tc in [LapHREG,ExpUPHREG,LapUPHREG]:
                test_class = tc
                class_name= test_class.__name__
                ext = f".{class_name.lower()}"
                datasets_dir=f"{dataset}_{class_name}"
                build_DP_from_file(dataset,exp)
                avgs = test_DP(dataset,exp)
            ...
            avg = test_base(dataset,exp)
            plt.axhline(avg,label=class_name,color="black",linestyle="--")
            ...
            build_base_from_file(dataset,exp)
            avg = test_base(dataset,exp)
            plt.axhline(avg,label=class_name,color="blue",linestyle="--")  
